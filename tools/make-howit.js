...
// Flat dark+gold design that matches the leaderboard page — 3 numbered steps + prize row + CTA.
...
function card(x, num, title, lines) {
  ...
  let t = `
    <rect x="${x}" y="${cardY}" width="${cw}" height="${cardH}" rx="20" fill="#11223d" fill-opacity="0.95" stroke="#274168" stroke-width="1.5"/>
    ...
    <text x="${x + 92}" y="${by + 12}" font-size="31" font-weight="bold" fill="#ffffff">${title}</text>`;
  lines.forEach((ln, i) => { t += `<text x="${x + 32}" y="${cardY + 122 + i * 33}" font-size="21" fill="#aebfd2">${ln}</text>`; });
  return t;
}
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <g text-anchor="middle" font-family="DejaVu Sans">
    <text x="600" y="80" font-size="26" font-weight="bold" letter-spacing="11" fill="#f4c742">P L O T L A N D S</text>
    <text x="600" y="152" font-size="62" font-weight="bold" fill="#ffffff">HOW IT WORKS</text>
    <text x="600" y="194" font-size="23" fill="#9fb6cf">$1000 Season Showdown   ·   free to play   ·   7-day season</text>
  </g>
