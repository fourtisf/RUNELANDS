...
const DEFS = `<defs>
  <linearGradient id="bg" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#0c1a2e"/><stop offset="1" stop-color="#0a1322"/></linearGradient>
  <linearGradient id="gold" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#ffe79a"/><stop offset="0.5" stop-color="#f4c742"/><stop offset="1" stop-color="#e0a52a"/></linearGradient>
</defs>`;
const frame = () => `
  <rect width="${W}" height="${H}" fill="url(#bg)"/>
  <rect x="0" y="0" width="${W}" height="6" fill="url(#gold)"/>
  <rect x="15" y="15" width="${W - 30}" height="${H - 30}" rx="22" fill="none" stroke="#21385c" stroke-width="1.5"/>`;
const header = (sub) => `
  <text x="600" y="74" text-anchor="middle" font-size="24" font-weight="bold" letter-spacing="10" fill="#f4c742" font-family="${F}">P L O T L A N D S</text>
...
function step(num, title, lines) {
  ...
  let b = header('$1000 Season Showdown')
    ...
    + `<circle cx="${cx}" cy="${cy}" r="${r - 17}" fill="#11223d"/>`
    + `<text x="${cx}" y="${cy + 50}" text-anchor="middle" font-size="150" font-weight="bold" fill="url(#gold)" font-family="${F}">${num}</text>`
    + `<text x="410" y="266" font-size="66" font-weight="bold" fill="#ffffff" font-family="${F}">${title}</text>`;
  lines.forEach((ln, i) => { b += `<text x="414" y="${330 + i * 44}" font-size="27" fill="#aebfd2" font-family="${F}">${ln}</text>`; });
...
(function prizes() {
  ...
  b += `<text x="600" y="182" text-anchor="middle" font-size="38" font-weight="bold" letter-spacing="5" fill="#ffffff" font-family="${F}">PRIZE POOL</text>`;
  ...
  tiers.forEach((t, i) => {
    const x = sx + i * (pw + pg);
    b += `<rect x="${x}" y="${py}" width="${pw}" height="${ph}" rx="16" fill="#11223d" stroke="#2c4870" stroke-width="1.5"/>`
      + `<text x="${x + pw / 2}" y="${py + 34}" text-anchor="middle" font-size="22" font-weight="bold" fill="#9fb6cf" font-family="${F}">${t[0]}</text>`
...
(function cta() {
  const b = header('')
    + `<text x="600" y="206" text-anchor="middle" font-size="42" font-weight="bold" letter-spacing="8" fill="#ffffff" font-family="${F}">SEASON SHOWDOWN</text>`
